...
impl Genesis {
	pub fn new(rom: &Vec<u8>) -> Genesis {
		...
		let cpu = cpu::CPU::new(&mut genesis);
	...
	pub fn advance_cycle(&mut self) {
		...
		if self.vdp.countdown == 0 {
			self.vdp.advance_cycle(&mut self.cpu.as_mut().unwrap());
		}
		self.vdp.countdown -= 1;
	}
}
impl Motorola68KBus for Genesis {
	fn read_u8(&mut self, address: u32) -> u8 {
		...
		match address_index {
			...
			0xC00000 | 0xC00002 => self.vdp.external_read_upper(),
			0xC00001 | 0xC00003 => self.vdp.external_read_lower(),
	...
	fn assert_interrupt(&mut self, level: u16) {
		self.cpu.as_mut().unwrap().assert_interrupt(level);
	}
}
